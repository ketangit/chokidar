...
void I2CPortDebounce::init(int devId, char portNameA, char portNameB, void(*func)(bool, uint8_t, char *portName)) {
    ...
    if(_fileHandle < 0) {
        ...
        syslog(LOG_INFO, "Initialize I2C deviceId=%d, fileHandle=%d portNames=[%s,%s]\n", 
            _deviceId, _fileHandle, _pDebouncePortA->getPortName(), _pDebouncePortB->getPortName());
    }	
}
...
void I2CPortDebounce::update(void) {
    ...
    if (_valuePortA != valA) {
        ...
        syslog(LOG_INFO, "Received I2C deviceId=%d, portName=%s, portValue=%u\n", _deviceId, _pDebouncePortA->getPortName(), _valuePortA);
    }
